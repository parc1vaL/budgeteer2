...
public static class BudgetEndpoints
{
    public static void MapBudgets(this IEndpointRouteBuilder app)
    {
        ...
        app.MapPut(
                ...
                (int year, int month, int categoryId, [FromBody] decimal amount, BudgetService service,
                        CancellationToken cancellationToken)
                    => service.CreateOrUpdateBudget(
                        new CreateOrUpdateBudgetRequest
                        {
                            Year = year, Month = month, CategoryId = categoryId, Amount = amount
                        }, cancellationToken))
